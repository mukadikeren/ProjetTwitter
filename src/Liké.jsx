...
export default function Liké() {
    const [Imgliké, setImgliké] = useState(false)
    const [liké, setliké] = useState (0)
    const handlesumit = () => {
        setImgliké(!Imgliké)
        liké ? setliké(liké - 1) : setliké(liké + 1)
    }

    const likéIcon = Imgliké ? 'src/images/heart-solid.svg' : 'src/images/heart-regular.svg';

    return (

        <div className=" liké">
            <img className="hover-3" onClick={handlesumit} src={likéIcon} alt="" />
